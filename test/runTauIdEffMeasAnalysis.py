...
tauIds = {
    ...
    'tauDiscrHPStight' : {
        ...
        legendEntry = cms.string("HPS Tight"),
        markerStyleData = 22,
        markerStyleSim = 26,
        color = 618
    },
    ...
    'tauDiscrHPSlooseDBcorr'  : {
        ...
        legendEntry = cms.string("HPS Loose"),
        markerStyleData = 20,
        markerStyleSim = 24,
        color = 418
    },
    'tauDiscrHPSmediumDBcorr' : {
        ...
        legendEntry = cms.string("HPS Medium"),
        markerStyleData = 21,
        markerStyleSim = 25,
        color = 807
    },
...
binning = {
    ...
    'tauPt' : {
        ...
        xAxisTitle = cms.string("P_{T}^{#tau}")
    },
    'tauEta' : {
        'tauEtaLt15' : {
            min =  0.0,
        ...
        xAxisTitle = cms.string("#eta_{#tau}")
    },
    'numVertices' : {
        'numVerticesLeq4' : {
            min =  -0.5,
    ...
    'sumEt' : {
        ...
        xAxisTitle = cms.string("#Sigma E_{T}")
    }
}
...
#
haddShellFileName_stage1 = os.path.join(outputFilePath, 'mergeTauIdEffHistograms_stage1_%.csh' % jobId)
...
for binVariable in binning.keys():
    for binName, binOptions in binning[binVariable]:
        retVal_fitTauIdEff = \
...
retVal_FWLiteTauIdEffPreselNumbers = \
  buildConfigFile_FWLiteTauIdEffPreselNumbers(inputFilePath, sampleZtautau, jobId_noTauSel, outputFilePath)
...
#
haddShellFileName_stage2 = os.path.join(outputFilePath, 'mergeTauIdEffHistograms_stage2_%.csh' % jobId)
haddInputFileNames_stage2 = outputFileNames_fitTauIdEffhaddInputFileNames
...
for binVariable in binning.keys():
    for binName, binOptions in binning[binVariable]:
        retVal_compTauIdEffFinalNumbers = \
          buildConfigFile_compTauIdEffFinalNumbers(haddOutputFileName_stage2, binName, tauIds.keys(), fitVariables, outputFileName)
        configFileNames_compTauIdEffFinalNumbers.append(retVal_compTauIdEffFinalNumbers['configFileName'])
        outputFileNames_compTauIdEffFinalNumbers.append(retVal_compTauIdEffFinalNumbers['outputFileName'])
#--------------------------------------------------------------------------------
...
#
haddShellFileName_stage3 = os.path.join(outputFilePath, 'mergeTauIdEffHistograms_stage3_%.csh' % jobId)
...
for binVariable in binning.keys():
    ...
    outputFileName_HPS = os.path.join(outputFilePath, 'makeTauIdEffFinalPlots_HPS_%s.eps' % binVariable)
    buildConfigFile_makeTauIdEffFinalPlots(haddOutputFileName_stage3, discriminators_HPS, binning[binVariable], fitVariables, outputFileName_HPS)
    ...
    outputFileName_HPSdbCorr = os.path.join(outputFilePath, 'makeTauIdEffFinalPlots_HPSdbCorr_%s.eps' % binVariable)
    buildConfigFile_makeTauIdEffFinalPlots(haddOutputFileName_stage3, discriminators_HPSdbCorr, binning[binVariable], fitVariables, outputFileName_HPSdbCorr)
    ...
    outputFileName_HPScombined = os.path.join(outputFilePath, 'makeTauIdEffFinalPlots_HPScombined_%s.eps' % binVariable)
    buildConfigFile_makeTauIdEffFinalPlots(haddOutputFileName_stage3, discriminators_HPScombined, binning[binVariable], fitVariables, outputFileName_HPScombined)
#--------------------------------------------------------------------------------
...
makeFile.write("\n")
for i, outputFileName in itemize(outputFileNames_FWLiteTauIdEffAnalyzer):
    makeFile.write("%s:\n" % outputFileName)
...
makeFile.write("\n")
for i, outputFileName in itemize(outputFileNames_fitTauIdEff):
    makeFile.write("%s: %s\n" % (outputFileName,
                                 haddOutputFileName_stage1))
...
makeFile.write("\n")
for i, outputFileName in itemize(outputFileNames_compTauIdEffFinalNumbers):
    makeFile.write("%s: %s\n" % (outputFileName,
                                 haddOutputFileName_stage2))
...
makeFile.write("%s: %s %s\n" % (haddOutputFileName_stage3,
                                ...
                                outputFileName_compTauIdEffFinalNumbers))
makeFile.write("\tsource %s\n" % haddShellFileName_stage3)
makeFile.write("\n")
for i, outputFileName in itemize(outputFileNames_makeTauIdEffFinalPlots):
    makeFile.write("%s: %s\n" % (outputFileName,
                                 haddOutputFileName_stage3))
