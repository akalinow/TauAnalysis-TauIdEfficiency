...
for hltPath in hltPaths:
    ...
    for jsonFileName, jsonFile in jsonFiles.items():
        MakeFile_dict[hltPath][jsonFile] = {}
        hltPath_abbr = hltPath
        hltPath_abbr = hltPath_abbr.replace("_v*", "")
        hltPath_abbr = hltPath_abbr.replace("*", "")    
        MakeFile_dict[hltPath][jsonFile]['outputFileName'] = 'lumiCalc_%s_%s.out' % (hltPath_abbr, jsonFileName)
        MakeFile_dict[hltPath][jsonFile]['lumiCalc_command'] = \
...
for hltPath in hltPaths:
    for jsonFileName in jsonFiles.keys():
        outputFileNames.append(MakeFile_dict[hltPath][jsonFile]['outputFileName'])
makeFile.write("all: parseLumiCalcOutput\n")
makeFile.write("\n")
for hltPath in hltPaths:
    for jsonFileName in jsonFiles.keys():
        makeFile.write("%s:\n" % MakeFile_dict[hltPath][jsonFile]['outputFileName'])
        makeFile.write("\t%s &> %s\n" %
          (MakeFile_dict[hltPath][jsonFile]['lumiCalc_command'],
           MakeFile_dict[hltPath][jsonFile]['outputFileName']))
makeFile.write("\n")    
